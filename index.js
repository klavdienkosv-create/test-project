...
let botOptions = { host: 'mc.play-fast.ru', port: 25565, username: 'pocohoco3000', version: '1.16.5', viewDistance: 'tiny', colorsEnabled: false, concurrency: 1 };
...
function showMenu() {
    ...
    console.log('---------------------------------------------------');
    console.log(' Запустить бота на сервер');
...
function createBot() {
    ...
    bot.on('spawn', () => {
        console.log(`[Система] Бот ${bot.username} зашел.`);
    ...
    bot.on('kick', () => { resetAllTimers(); isBotActive = false; if (rlInterface) rlInterface.close(); setTimeout(showMenu, 5000); });
    bot.on('end', () => { resetAllTimers(); isBotActive = false; if (rlInterface) rlInterface.close(); setTimeout(showMenu, 5000); });
